...
origins of the follow-up is specified through the corresponding arguments,
`time`, `id`, `event`, and `origin` in function `Recur`, where the argument
`event` takes negative values as censoring, and positive values as costs or event
indicators.  The function `Recur` serves as the formula response and contains a
...
`Recur` as response is specified in function `mcf`, the formula method for
