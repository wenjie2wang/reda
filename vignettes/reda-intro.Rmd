...
or so-called Nelson-Aalen estimator. The most functions in this package
are S4 methods that produce S4 class objects.

In this vignette, we mainly introduce the basic usage of the functions
provided in the package by some examples. The details of function snytax
...
What follows next is the examples of the functions to
[summarize](#summaryFit) the model fitted.
...
A brief summary of the fitted model is simply given by `show` method
...
especially useful if one wants a relatively concise summary
...
After estimation, we may simply plot the sample MCF by function `plotMcf`,
...
The example to estimate and plot
